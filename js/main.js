...
$(function() {
    ...
    let normalNavHeight = $('.barra').innerHeight();
    // Menu sigue al usuario
    ...
    $('nav.navegacion-principal > a').on('click', function(event) {
        ...
        if (window.mobilecheck()) {
            ...
            $('html, body').animate({
                scrollTop: $(section).offset().top-normalNavHeight
            }, 1000);
...
// of the main navigation bar, since this attribute is added when the slideToggle() function is used
// and messes up some of the CSS rules.
$(window).resize(function() {
    if (window.innerWidth >= 768) {
        $('nav.navegacion-principal').attr('style', '');
    }
});
